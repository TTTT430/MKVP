...
Version: 1.1 (Modified for Robustness Test)
"""
...
class TextNoiser:
    def __init__(self, noise_type='none', noise_ratio=0.0):
        """
        noise_type: 'none', 'delete' (随机删除), 'shuffle' (随机乱序)
        noise_ratio: 0.0 ~ 1.0
        """
    ...
    def apply(self, text):
        ...
        if self.noise_type == 'delete':
            keep_prob = 1.0 - self.noise_ratio
            new_words = [w for w in words if random.random() < keep_prob]
            if not new_words: new_words = [random.choice(words)]
            return " ".join(new_words)

        elif self.noise_type == 'shuffle':
            num_swaps = int(n * self.noise_ratio)
            if num_swaps == 0 and self.noise_ratio > 0 and n > 1:
                num_swaps = 1
...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms, data_type,
                 ...
                 noise_type='none', noise_ratio=0.0):
        ...
        self.noiser = TextNoiser(noise_type, noise_ratio)

        for data in file_content:
            self.data_id_list.append(data['id'])

            raw_text = data['text']
            if self.data_type != 1 and noise_type != 'none':
                processed_text = self.noiser.apply(raw_text)
            else:
                processed_text = raw_text

            self.text_list.append(processed_text)
            self.label_list.append(data['emotion_label'])
            self.aspect_words_list.append(data.get('aspect_words', []))
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # 防止图片损坏报错，生成纯黑图替代
            image_read = Image.new('RGB', (224, 224), (0, 0, 0))
        ...
        return self.text_to_id[index], image_origin, self.label_list[index], self.text_translation_to_id[self.data_id_list[index]], image_augment, self.text_list[index], self.aspect_words_list[index]


class Collate():
    def __init__(self, opt, pad_token_id=0):
    ...
    def _collate(self, batch_data):
        ...
        image_origin = torch.stack([b[1] for b in batch_data])
        label = torch.LongTensor([b[2] for b in batch_data])
        text_translation_to_id = [torch.LongTensor(b[3]) for b in batch_data]
        image_augment = torch.stack([b[4] for b in batch_data])
        raw_texts_batch = [b[5] for b in batch_data]
        aspect_words_batch = [b[6] for b in batch_data]
        ...
        if max_length < self.min_length:
            text_to_id[0] = torch.cat(
                (text_to_id[0], torch.LongTensor([self.pad_token_id] * (self.min_length - text_to_id[0].size(0)))))
        ...
        if max_translation_length < self.min_length:
            text_translation_to_id[0] = torch.cat((text_translation_to_id[0], torch.LongTensor(
                [self.pad_token_id] * (self.min_length - text_translation_to_id[0].size(0)))))
            max_translation_length = self.min_length

        text_to_id = run_utils.pad_sequence(text_to_id, batch_first=True, padding_value=self.pad_token_id)
        text_translation_to_id = run_utils.pad_sequence(text_translation_to_id, batch_first=True,
        ...
        return text_to_id, torch.LongTensor(bert_attention_mask), image_origin, torch.LongTensor(
            ...
            raw_texts_batch, aspect_words_batch
...
def data_process(opt, data_path, text_tokenizer, photo_path, data_type,
    ...
    dataset = SentenceDataset(opt, data_path, text_tokenizer, photo_path,
                              transform_train if data_type == 1 else transform_test_dev,
                              data_type,
    ...
    data_loader = DataLoader(dataset, batch_size=opt.acc_batch_size,
                             ...
                             collate_fn=Collate(opt, pad_token_id=text_tokenizer.pad_token_id),
